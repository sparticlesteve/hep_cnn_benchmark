...
def parse_arguments():
    ...
    parser.add_argument('--num_ps', type=int, default=0,
                        help='specify the number of parameters servers')
...
def train_loop(sess, train_step, global_step,
               #optlist,
    ...
    while not sess.should_stop():
        ...
        if args['create_summary']:
            _, gstep, summary, tmp_loss = sess.run([train_step, global_step,
                                                    train_summary, loss_fn],
                                                   feed_dict=feed_dict)
        else:
            _, gstep, tmp_loss = sess.run([train_step, global_step, loss_fn],
                                          feed_dict=feed_dict)
        ...
        if (gstep % args['display_interval']) == 0:
            print(time.time(), 'REPORT rank', args['task_index'],
                  'global step %d., average training loss %g (%.3f sec/batch)' %
                  (gstep, train_loss/float(train_batches), train_time/float(train_batches)))
        
        #check if epoch is done
        if trainset._epochs_completed > epochs_completed:
            epochs_completed = trainset._epochs_completed
            print(time.time(), 'COMPLETED rank', args['task_index'],
            ...
            print(time.time(), 'COMPLETED epoch %d, average validation loss %g' %
                  (epochs_completed, validation_loss/float(validation_batches)))
            validation_accuracy = sess.run(accuracy_fn[0])
            print(time.time(), 'COMPLETED epoch %d, average validation accu %g' %
                  (epochs_completed, validation_accuracy))
            validation_auc = sess.run(auc_fn[0])
            print(time.time(), 'COMPLETED epoch %d, average validation auc %g' %
...
if args['num_tasks'] > 1:
    args['cluster'], args['server'], args['task_index'], args['num_workers'], args['node_type'] = (
        sc.setup_slurm_cluster(num_ps=args['num_ps'])) 
...
if not args['batch_size_per_node']:
    args['train_batch_size_per_node'] = int(
        args['train_batch_size'] / float(args['num_workers']))
    args['validation_batch_size_per_node'] = int(
        args['validation_batch_size'] / float(args['num_workers']))
else:
...
if (args['node_type'] == 'worker'):
    ...
    if args['arch']=='hsw':
        ...
        raise ValueError('Please specify a valid architecture with arch ' +
                         '(allowed values: hsw, knl, gpu)')
    ...
    print('Rank', args['task_index'], ': using', num_inter_threads,
          '-way task parallelism with', num_intra_threads, '-way data parallelism.')
...
if args['node_type'] == 'worker':
    ...
    if not args['dummy_data']:
        #training files
        trainfiles = [args['inputpath'] + '/' + x for x in os.listdir(args['inputpath'])
                      if 'train' in x and (x.endswith('.h5') or x.endswith('.hdf5'))]
        trainset = bc.DataSet(trainfiles, args['num_workers'], args['task_index'],
                              split_filelist=True, split_file=False,
                              data_format=args['conv_params']['data_format'])

        #validation files
        validationfiles = [args['inputpath'] + '/' + x for x in os.listdir(args['inputpath'])
                           if 'val' in x and (x.endswith('.h5') or x.endswith('.hdf5'))]
        validationset = bc.DataSet(validationfiles, args['num_workers'], args['task_index'],
                                   split_filelist=True, split_file=False,
                                   data_format=args['conv_params']['data_format'])
    else:
        #training files and validation files are just dummy sets then
        trainset = bc.DummySet(input_shape=args['input_shape'], samples_per_epoch=10000,
                               task_index=args['task_index'])
        validationset = bc.DummySet(input_shape=args['input_shape'], samples_per_epoch=1000,
                                    task_index=args['task_index'])
...
metafilelist = [args['modelpath'] + '/' + x for x in os.listdir(args['modelpath'])
                if x.endswith('.meta')]
if not metafilelist:
    ...
    with args['graph'].as_default():
        ...
        with tf.device(args['device']):
            ...
            if args['mode'] == 'sync' and args['num_tasks'] > 1:
                ...
                #if sync we make a data structure that will aggregate the
                # gradients form all tasks (one task per node in this case)
            ...
            if args['create_summary']:
                ...
                hooks.append(tf.train.StepCounterHook(every_n_steps=100,
                                                      output_dir=args['logpath']))
            ...
            print('Rank', args['task_index'],
                  ': starting training using ' + args['optimizer'] + ' optimizer')
            ...
            with tf.train.MonitoredTrainingSession(config=sess_config, 
                ...
                train_loop(sess, train_step, global_step, args, trainset, validationset)
